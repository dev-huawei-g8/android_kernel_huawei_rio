...
static int ap3426_debug_mask= 1;
...
static int ap3426_process_data(struct ap3426_data *di, int als_ps)
{
	...
	if (als_ps) { /* process als value */
		...
		AP3426_DEBUG("%s,line %d:lux:%d als_data:0x%x-0x%x\n",__func__,__LINE__,
		...
		AP3426_DEBUG("%s,line %d:previous als_gain:%d\n", __func__,__LINE__,di->als_gain);
		...
		AP3426_DEBUG("%s,line %d:reprt work ps_data:%d\n",__func__,__LINE__, tmp);
		...
		AP3426_DEBUG("%s,line %d:ps report exit\n",__func__,__LINE__);
	}
