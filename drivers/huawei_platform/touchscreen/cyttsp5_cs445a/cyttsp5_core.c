		...
		u16 irq, size_t xfer_buf_size)
{
	...
	rc = add_easy_wakeup_interfaces(cd->dev);
	if (rc < 0) {
	...
	if (cd->glove_support) {
		rc = add_glove_interface(cd->dev);
		if (rc < 0) {
			tp_log_err( "%s: Error, glove init sysfs fail! \n", __func__);
			goto error_startup_btn;
		}
	}

	if (cd->holster_support) {
		rc = add_holster_interface(cd->dev);
		if (rc < 0) {
