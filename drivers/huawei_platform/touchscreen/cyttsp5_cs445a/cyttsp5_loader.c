...
{
	if (NULL == panel_name || length <= 0) {
		tp_log_err("%s %d:Missing input parameter\n", __func__, __LINE__);
		return 0;
	}

	memset(panel_name, 0, length);
	switch(panel_id)
	{
	case FW_OFILM:
		strncpy(panel_name, "Ofilm", length);
		break;
	case FW_EELY:
    ...
    case FW_GIS:
        ...
        break;
	case FW_JUNDA:
		strncpy(panel_name, "JunDa", length);
		break;
	case FW_LENSONE:
	...
	tp_log_info("%s %d:Panel name is %s\n", __func__, __LINE__, panel_name);
	return strnlen(panel_name, length);
}
