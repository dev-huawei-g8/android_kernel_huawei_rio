...
#ifdef CONFIG_HUAWEI_KERNEL
	/*Add CMD13 to clear up non-processed CMD after Tuning operation to avoid crc error*/
	if (CID_MANFID_SAMSUNG == host->mmc->card->cid.manfid)
	{
		EMMCSD_LOG_INFO("%s: %s: send CMD13 after tuning oprtation\n",
				mmc_hostname(mmc), __func__);
		get_card_status(host->mmc->card, &status, 3);
	}
#endif
