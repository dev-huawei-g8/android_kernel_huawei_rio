...
#ifdef MSM_8916_PLATFORM
#define RECORD_PHONE "record on msm 8916 platform"
#endif
#ifdef MSM_8960_PLATFORM
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
...
#ifdef CONFIG_HUAWEI_KERNEL
    int has_usb_connect = 0;

    has_usb_connect = is_usb_chg_exist();
    if(has_usb_connect < 0){
        printk(KERN_ERR "ate[E]: USB was not plug!");
        return 0;
    }
#endif
    ...
    if (0 != is_record) {
        ...
        ate_dt->ie_times = 0;
#ifdef CONFIG_HUAWEI_KERNEL
        ate_dt->has_usb_disconnect = !has_usb_connect;
#else
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    ret = snprintf(ate_dt->iedt_file_dir, sizeof(ate_dt->iedt_file_dir) - 1, "%s", buf);
...
static void default_set_input_device_filter_array(void )
{
    ...
    ate_dt->idf.input_event_type[1] = TOUCH_BUTTON_EVENT;
    strncpy(ate_dt->idf.dev_name[2], "hi6xxx_on", MAX_SIZE_OF_DEV_NAME-1); /* power key */
    ate_dt->idf.input_event_type[2] = POWER_KEY_EVENT;
...
#endif
#ifdef MSM_8916_PLATFORM
    strncpy(ate_dt->idf.dev_name[0], "ft6x06_i2c_adapter", MAX_SIZE_OF_DEV_NAME-1);
    ate_dt->idf.input_event_type[0] = TOUCH_EVENT;
    strncpy(ate_dt->idf.dev_name[1], "ms-msg21xx", MAX_SIZE_OF_DEV_NAME-1);
    ate_dt->idf.input_event_type[1] = TOUCH_EVENT;
    strncpy(ate_dt->idf.dev_name[2], "synaptics_dsx_i2c", MAX_SIZE_OF_DEV_NAME-1);
    ate_dt->idf.input_event_type[2] = TOUCH_EVENT;
    strncpy(ate_dt->idf.dev_name[3], "Goodix-TS", MAX_SIZE_OF_DEV_NAME-1);
    ate_dt->idf.input_event_type[3] = TOUCH_EVENT;
    strncpy(ate_dt->idf.dev_name[4], "qpnp_pon", MAX_SIZE_OF_DEV_NAME-1); /* power key */
    ate_dt->idf.input_event_type[4] = POWER_KEY_EVENT;
    strncpy(ate_dt->idf.dev_name[5], "gpio-keys", MAX_SIZE_OF_DEV_NAME-1);
    ate_dt->idf.input_event_type[5] = SOUND_KEY_EVENT;
    ate_dt->idf.filer_total = 6;
#else
    strncpy(ate_dt->idf.dev_name[0], "synaptics_dsx_i2c", MAX_SIZE_OF_DEV_NAME-1);
    ate_dt->idf.input_event_type[0] = TOUCH_EVENT;
    strncpy(ate_dt->idf.dev_name[2], "qpnp_pon", MAX_SIZE_OF_DEV_NAME-1); /* power key */
    ate_dt->idf.input_event_type[2] = POWER_KEY_EVENT;
    strncpy(ate_dt->idf.dev_name[3], "gpio-keys", MAX_SIZE_OF_DEV_NAME-1);
    ate_dt->idf.input_event_type[3] = SOUND_KEY_EVENT;
    ate_dt->idf.filer_total = SOUND_KEY_EVENT+1;
#endif
}
...
static ssize_t get_virtualkey_config(void)
{
    ...
    buf_vkey_size = snprintf(c_buf, sizeof(c_buf),
    ...
    for(i = ate_dt->virtualkey.total_dev = 0; i<500 &&
        ...
        if(!strcmp(p_char, __stringify(EV_KEY))) {
            ...
            printk(KERN_ERR "the type of key is error\n");
            return -1;
        }
