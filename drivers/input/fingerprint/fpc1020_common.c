...
/* echo debug_level > sys/devices/platform/fingerprint/diag/debug_mask */
u8 g_fpc_debug_mask = FPC_INFO;
...
static const struct chip_struct chip_data[] = {
    ...
    {FPC1020_CHIP_1140B, 0x140b, 1, FPC1140_ROWS, FPC1140_COLUMNS, FPC102X_ADC_GROUP_SIZE},
    {FPC1020_CHIP_1140C, 0x140c, 1, FPC1140_ROWS, FPC1140_COLUMNS, FPC102X_ADC_GROUP_SIZE},
...
int fpc1020_setup_defaults(fpc1020_data_t *fpc1020)
{
    ...
    if (fpc1020->chip.type == FPC1020_CHIP_1020A) {
        if (gpio_is_valid(fpc1020->moduleID_gpio))
        {
            ...
            {
                fpc_log_info("fingerprint module LO low\n");
    ...
    } else if (fpc1020->chip.type == FPC1020_CHIP_1140C) {
        ptr = (fpc1020->chip.revision == 1) ? &fpc1020_setup_default_1140 : NULL;
    }else {
        ptr = NULL;
    }
...
/* add debug information when read irq fail after gpio reset */
/* -------------------------------------------------------------------- */
int fpc1020_gpio_reset(fpc1020_data_t *fpc1020)
{
    ...
    while (counter) {
        ...
        if (gpio_get_value(fpc1020->reset_gpio) != 0) {
            fpc_log_err("Unable to pull RST_N low!\n");
        }

        gpio_set_value(fpc1020->reset_gpio, 1);
        udelay(FPC1020_RESET_HIGH2_US);

        if (gpio_get_value(fpc1020->reset_gpio) != 1) {
            fpc_log_err("Unable to pull RST_N up!\n");
        }
        ...
        if (!error) {
            ...
            /* read fpc status reg 0x1f */
            status = fpc1020_read_status_reg(fpc1020);
            fpc_log_err("fpc status register value = 0x%x\n",status);
            /* read fpc irq reg 0xff */
            status = fpc1020_read_irq_for_reset(fpc1020, false);
            fpc_log_err("fpc irq register value = 0x%x\n",status);
            /* do spi reset */
            error = fpc1020_spi_reset(fpc1020);
            udelay(1250);
        }
    }
    return error;
...
int fpc1020_reset(fpc1020_data_t *fpc1020)
{
    ...
    if (error < 0) {
        fpc_log_err("fpc1020_read_irq_for_reset failed, error = %d\n", error);
        goto out;
    }

    if (error != FPC_1020_IRQ_REG_BITS_REBOOT) {
        fpc_log_err("IRQ register, expected 0x%x, got 0x%x\n", FPC_1020_IRQ_REG_BITS_REBOOT, (u8)error);
        error = -EIO;
        goto out;
    }
    ...
    if (error)
        fpc_log_err("IRQ pin, not low after clear\n");
...
static int fpc1020_check_hw_id_extended(fpc1020_data_t *fpc1020)
{
    ...
    if (error < 0) {
        fpc_log_err("Unable to check chip revision, error = %d\n", error);
    }
...
static int fpc1020_hwid_1020a(fpc1020_data_t *fpc1020)
{
    ...
    fpc_log_debug("m1,m2 = %d,%d %s rev: %d\n",
        m1, m2,
...
{
    fpc_log_debug("ENTER! chip.type = %d\n", fpc1020->chip.type);
    ...
    fpc_log_err("chip type is not match! chip.type = %d", fpc1020->chip.type);
    return -EINVAL;
}
...
int fpc1020_write_cb_test_setup_1140(fpc1020_data_t *fpc1020, bool invert)
{
    ...
    fpc_log_info("ENTER! pattern = 0x%x\n", temp_u16);
...
int fpc1020_write_sensor_checkerboard_setup_1020(fpc1020_data_t *fpc1020, u16 pattern)
{
    ...
    fpc_log_info("Enter! mux = %d, chip.revision = %d\n", mux, fpc1020->chip.revision);
...
int fpc1020_wait_for_irq(fpc1020_data_t *fpc1020, int timeout)
{
    ...
    if (result < 0) {
        fpc_log_err("wait_event_interruptible interrupted by signal\n");
        return result;
    }
...
int fpc1020_read_irq(fpc1020_data_t *fpc1020, bool clear_irq)
{
    ...
    if (error >=0 && irq_status == FPC_1020_IRQ_REG_BITS_REBOOT) {
        fpc_log_err("FAILED! irq_status = 0x%x\n", irq_status);
        error = -EIO;
    }
              ...
              fpc1020_reg_access_t *reg_data)
{
    ...
    do {
        ...
        if (0 == error) {
            ...
            fpc_log_err("spi_sync failed, error = %d\n", error);
            msleep(100);
            spi_failed_times++;
        }
...
int fpc1020_dummy_capture(fpc1020_data_t *fpc1020)
{
    ...
    error = fpc1020_read_irq(fpc1020, true);
    if (error < 0) {
        fpc_log_err("fpc1020_read_irq failed, error = %d", error);
        return error;
    }
    error = fpc1020_cmd(fpc1020,
    ...
    if (error < 0) {
        fpc_log_err("fpc1020_cmd failed, error = %d", error);
        return error;
    }
    return error;
}
    ...
    while (1) {
        ...
        if (error >= 0) {
            ...
            if (error &
                ...
                FPC_1020_IRQ_REG_BIT_COMMAND_DONE)) {
                fpc_log_debug("Finger Down!\n");
                error = 0;
            } else {
                fpc_log_err("Unexpected IRQ = %d\n", error);
                error = -EIO;
            }
            return error;
        }
...
int fpc1020_wait_finger_present(fpc1020_data_t *fpc1020)
{
    ...
    while (1) {
        ...
        if (error < 0) {
            ...
            fpc_log_err("fpc1020_wait_for_irq error = %d\n", error);
            return error;
        }
...
int fpc1020_check_finger_present_raw(fpc1020_data_t *fpc1020)
{
    ...
    if (error < 0) {
        fpc_log_err("fpc1020_cmd failed, error = %d\n", error);
        return error;
    }
...
int fpc1020_check_finger_present_sum(fpc1020_data_t *fpc1020)
{
    ...
    if (zones < 0) {
        fpc_log_err("invaild zones, zones = %d\n", zones);
        return zones;
    }
    else {
        zones &= mask;
...
int fpc1020_sleep(fpc1020_data_t *fpc1020, bool deep_sleep)
{
    ...
    if (error) {
        fpc_log_err("%s sleep command failed, error = %d\n",
    ...
    while (!sleep_ok && retries && (error >= 0)) {
        ...
        if (error < 0) {
            fpc_log_err("%s sleep read status failed, error = %d\n",
                (deep_sleep)? str_deep : str_regular,
                error);
        } else {
            error &= status_mask;
    ...
    if (sleep_ok) {
        fpc_log_debug("%s sleep OK!\n",
            (deep_sleep)? str_deep : str_regular);
        return 0;
    } else {
        fpc_log_debug("%s sleep FAILED!\n",
            (deep_sleep)? str_deep : str_regular);
                ...
                u32 buff_size)
{
    ...
    if ((offset + (int)image_size_bytes) > buff_size) {
        fpc_log_err("Image buffer too small for offset: %d (max %d bytes)",
            offset,
            buff_size);
...
u32 fpc1020_calc_image_size(fpc1020_data_t *fpc1020)
{
   unsigned int image_byte_size = fpc1020->setup.capture_row_count *
    ...
    fpc_log_debug("Rows %d->%d, Cols %d->%d (%d bytes)\n",
