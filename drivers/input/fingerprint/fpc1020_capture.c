...
int fpc1020_write_test_setup(fpc1020_data_t *fpc1020, u16 pattern)
{
    ...
    switch (fpc1020->chip.type) {
        ...
        case FPC1020_CHIP_1140C:
            if (0xaa55u == pattern){
                error =fpc1020_write_cb_test_setup_1140(fpc1020, false);
            }
...
int fpc1020_capture_task(fpc1020_data_t *fpc1020)
{
    ...
    fpc_log_debug("Start capture, capture_mode = %d, (%d frames)\n", mode, capture_count);
...
int cal_thredhold(fpc1020_data_t *fpc1020)
{
    ...
    if (error < 0) {
        fpc_log_err("fpc1020_fetch_image failed, error = %d\n", error);
        goto out_error;
    }
    for (i = 0; i<64 ; i++)
    {
    ...
    sum = sum>>1;
    fpc_log_debug("fingerprint sum = %d\n", sum);
out_error:
...
int fpc1020_capture_wait_finger_down(fpc1020_data_t *fpc1020)
{
    ...
    if( fpc1020->setup.finger_auto_threshold ){
        ...
        if (error < 0) {
            fpc_log_err("[0] fpc1020_capture_set_crop failed, error = %d\n", error);
            goto out_error;
        }
        error = cal_thredhold(fpc1020);
        if (error < 0) {
            fpc_log_err("[0] cal_thredhold failed, error = %d\n", error);
            goto out_error;
        }
        thold = error;
        error = fpc1020_capture_set_crop(fpc1020,fpc1020->setup.wakeup_detect_cols[1],1,fpc1020->setup.wakeup_detect_rows[1],8);
        if (error < 0) {
            fpc_log_err("[1] fpc1020_capture_set_crop failed, error = %d\n", error);
            goto out_error;
        }
        error = cal_thredhold(fpc1020);
        if (error < 0) {
            fpc_log_err("[1] cal_thredhold failed, error = %d\n", error);
            goto out_error;
        }
        if(error > thold)
            thold = error;
        ...
        fpc_log_debug("fp_thredhold=%d default_hold=%d det1=[%d,1,%d,8] det2=[%d,1,%d,8]\n",\
...
int fpc1020_capture_wait_finger_up(fpc1020_data_t *fpc1020)
{
    ...
    while (!finger_up && (error >= 0)) {
        ...
        if(i > 100){
            i = 0;
            fpc_log_info("finger present sum = %d\n", error);
        }
...
int fpc1020_capture_settings(fpc1020_data_t *fpc1020, int select)
{
    ...
    fpc_log_info("ENTER! select = %d\n", select);

    if (select >= FPC1020_BUFFER_MAX_IMAGES) {
        error = -EINVAL;
        fpc_log_err("select great than or equal to MAX IMAGES, error = %d\n", error);
        goto out_err;
    }
