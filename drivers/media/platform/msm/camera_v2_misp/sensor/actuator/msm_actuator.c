...
#if 1
	spi_reg.opcode = ISPCMD_BASIC_SET_AF_STEPS;
	spi_reg.param = param;
	param[0] = 1;
	steps=(uint16_t*)&param[1];
	*steps = (uint16_t)dest_step_pos;
	spi_reg.size = sizeof(param);
	param[9] = 2;
	pr_err("tell miniisp move to dest_step_pos:%d",*steps);
	rc=misp_write_cmd(spi_reg.opcode,spi_reg.param,spi_reg.size);
#else
	reg_setting.reg_setting = a_ctrl->i2c_reg_tbl;
	reg_setting.data_type = a_ctrl->i2c_data_type;
	reg_setting.size = a_ctrl->i2c_tbl_index;
...
static int msm_actuator_init(struct msm_actuator_ctrl_t *a_ctrl)
{
	...
	a_ctrl->cur_vcm_code = 0;
	CDBG("Exit\n");
	return rc;
}

static int32_t msm_actuator_set_vcm_code(struct msm_actuator_ctrl_t *a_ctrl,
										uint16_t vcm_code)
{
	int32_t rc = 0;
	uint8_t param[10]={0};
	...
	param[0] = 1;  //primary rear camera
	memcpy(&(param[1]), &vcm_code, sizeof(vcm_code));
	param[9] = NORMALIZED_HW_STEP;

	pr_err("Send cmd to mini ISP: move actuator to: %d \n", vcm_code);
	rc=misp_write_cmd(ISPCMD_BASIC_SET_AF_STEPS,
						param,
						sizeof(param));
	if(!rc)
		a_ctrl->cur_vcm_code = vcm_code;

	CDBG("Exit\n");
	return rc;
}
/*This func is to move actuator to original postion slowly
 * to avoid noisy tick sound.
 * TODO: use vcm otp to optimize -- not works well for now
 */
static int32_t msm_actuator_hw_park_lens(struct msm_actuator_ctrl_t *a_ctrl)
{
	int32_t rc = 0;
	uint16_t next_vcm_code = 0;
	uint16_t park_start_code = 200, park_step_code = 40;

	pr_err("cur vcm=%d\n", a_ctrl->cur_vcm_code);

	if(a_ctrl->cur_vcm_code > park_start_code){
		rc = msm_actuator_set_vcm_code(a_ctrl, park_start_code);
		if (rc < 0) {
			pr_err("Failed actuator set_vcm_code %d\n", rc);
			return rc;
		}
		usleep_range(10000, 11000);
	}

	next_vcm_code = a_ctrl->cur_vcm_code;
	while (next_vcm_code) {
		next_vcm_code = next_vcm_code > park_step_code ?
			next_vcm_code - park_step_code : 0;

		pr_debug("%s, %d , next code=%d\n", __func__, __LINE__, next_vcm_code);
		rc = msm_actuator_set_vcm_code(a_ctrl,
			next_vcm_code);
		if (rc < 0) {
			pr_err("Failed actuator set_vcm_code %d\n", rc);
			return rc;
...
static int msm_actuator_close(struct v4l2_subdev *sd,
	...
	msm_actuator_hw_park_lens(a_ctrl);
	if (a_ctrl->act_device_type == MSM_CAMERA_PLATFORM_DEVICE) {
		rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_util(
	...
	struct file *file, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_MSM_ACTUATOR_CFG32:
		...
		case CFG_SET_VCM_CODE:
			actuator_data.cfgtype = u32->cfgtype;
			actuator_data.cfg.vcm_code = u32->cfg.vcm_code;
			break;
