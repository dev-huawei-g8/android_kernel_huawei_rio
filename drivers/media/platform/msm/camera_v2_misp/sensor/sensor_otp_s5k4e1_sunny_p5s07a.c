...
struct  s5k4e1_sunny_otp_struct {
...
typedef enum tags5k4e1_sunny_otp_location
{
    S5K4E1_SUNNY_OTP_BEGIN = 0,
    S5K4E1_SUNNY_OTP_310D_LSB,//reg 0x310d lsb
    S5K4E1_SUNNY_OTP_310E_MSB,//reg 0x310e msb
    S5K4E1_SUNNY_OTP_310E_LSB,//reg 0x310e lsb
    S5K4E1_SUNNY_OTP_310F_MSB,//reg 0x310f msb
    S5K4E1_SUNNY_OTP_310F_LSB,//reg 0x310f lsb
    S5K4E1_SUNNY_OTP_MAX,//no otp
}s5k4e1_sunny_otp_location;

#define S5K4E1_SUNNY_LSB  0x0f
#define S5K4E1_SUNNY_MSB  0xf0

//otp Module info start layer
#define S5K4E1_SUNNY_LAYER_START      18
//otp Module info end layer
#define S5K4E1_SUNNY_LAYER_END        27
//awb otp start layer
#define S5K4E1_SUNNY_LAYER_AWB_START  28
//awb otp end layer
#define S5K4E1_SUNNY_LAYER_AWB_END    35

//number of otp info layers
#define S5K4E1_SUNNY_LAYER_NUMS   (S5K4E1_SUNNY_LAYER_AWB_END - S5K4E1_SUNNY_LAYER_START +1)

//AWB otp exist flag
#define S5K4E1_SUNNY_OTP_AWB          1
#define S5K4E1_SUNNY_OTP_ALL          (S5K4E1_SUNNY_OTP_AWB)

//R/G and B/G value of Golden Samples.
static  uint32_t rg_ratio_typical = 0x312;
static  uint32_t bg_ratio_typical = 0x2DE;
...
static uint32_t s5k4e1_sunny_otp_read_flag = 0 ;
...
/******************************************************************************
Function   :   s5k4e1_sunny_otp_debug
Description:   Only for debug use
******************************************************************************/
void s5k4e1_sunny_otp_debug(struct s5k4e1_sunny_otp_struct  otp_ptr)
{
...
/******************************************************************************
Function   :    s5k4e1_sunny_update_awb_gain
Description:    Update the RGB AWB gain to sensor
******************************************************************************/
void s5k4e1_sunny_update_awb_gain(struct msm_sensor_ctrl_t *s_ctrl,
                                             uint32_t r_gain,
                                             uint32_t g_gain,
                                             uint32_t b_gain)
{
...
/******************************************************************************
Function:       s5k4e1_sunny_read_otp_regs
Description:    According the locaion, read the otp mem
******************************************************************************/
void s5k4e1_sunny_read_otp_regs(struct msm_sensor_ctrl_t * s_ctrl,
                                          struct s5k4e1_sunny_otp_struct *current_otp,
                                          uint32_t reg,
                                          int isMsb)
{
	int layer =0;
	int i = 0;
	uint16_t reg_values[S5K4E1_SUNNY_LAYER_NUMS] = {0};
	int flag_true = 0;
	CMR_LOGD("%s reg:0x%x, isMsb:%d ", __func__, reg, isMsb);

	for (layer = S5K4E1_SUNNY_LAYER_START; layer <= S5K4E1_SUNNY_LAYER_AWB_END; layer++)
	{
		i = layer - S5K4E1_SUNNY_LAYER_START;
		...
		{
			reg_values[i] =  (reg_values[i] & S5K4E1_SUNNY_MSB) >> 4;
		}
		else//LSB
		{
			reg_values[i] =  reg_values[i] & S5K4E1_SUNNY_LSB;
		}
	...
	{
		//read successful!
		s5k4e1_sunny_otp_read_flag |= S5K4E1_SUNNY_OTP_AWB;
...
/******************************************************************************
Function   :    s5k4e1_sunny_read_awb_otp
Description:    According the locaion, read the otp mem
******************************************************************************/
static int s5k4e1_sunny_read_awb_otp(struct msm_sensor_ctrl_t * s_ctrl,
                                                s5k4e1_sunny_otp_location location,
                                                struct s5k4e1_sunny_otp_struct *current_otp)
{
	int rc = 0;
	int flag_flase = -1;
	...
	if (location >= S5K4E1_SUNNY_OTP_MAX)
	{
	...
	switch (location)
	{
		case S5K4E1_SUNNY_OTP_310D_LSB:
			s5k4e1_sunny_read_otp_regs(s_ctrl, current_otp, 0x310d, flag_flase);
			break;
		case S5K4E1_SUNNY_OTP_310E_MSB:
			s5k4e1_sunny_read_otp_regs(s_ctrl, current_otp, 0x310e, flag_true);
			break;
		case S5K4E1_SUNNY_OTP_310E_LSB:
			s5k4e1_sunny_read_otp_regs(s_ctrl, current_otp, 0x310e, flag_flase);
			break;
		case S5K4E1_SUNNY_OTP_310F_MSB:
			s5k4e1_sunny_read_otp_regs(s_ctrl, current_otp, 0x310f, flag_true);
			break;
		case S5K4E1_SUNNY_OTP_310F_LSB:
			s5k4e1_sunny_read_otp_regs(s_ctrl, current_otp, 0x310f, flag_flase);
			break;
		default:
			rc = -1;
			break;
...
/******************************************************************************
Function   :   s5k4e1_sunny_check_awb_otp_location
Description:   find which location otp lays in
******************************************************************************/
s5k4e1_sunny_otp_location s5k4e1_sunny_check_awb_otp_location(struct msm_sensor_ctrl_t * s_ctrl)
{
	uint16_t reg310d = 0;//reg 0x310d value
	uint16_t reg310e = 0;//reg 0x310e value
	uint16_t reg310f = 0;//reg 0x310f value

	uint16_t layer_data = S5K4E1_SUNNY_LAYER_START;
	...
	CMR_LOGD("%s read layer %d: reg310d:0x%x, reg310e:0x%x, reg310f:0x%x \n", __func__, layer_data, reg310d, reg310e, reg310f);
	//find which reg postion otp lays in, from right to left
	if (reg310d & S5K4E1_SUNNY_LSB)
	{
		return S5K4E1_SUNNY_OTP_310D_LSB;
	}
	else if (reg310e & S5K4E1_SUNNY_MSB)
	{
		return S5K4E1_SUNNY_OTP_310E_MSB;
	}
	else if (reg310e & S5K4E1_SUNNY_LSB)
	{
		return S5K4E1_SUNNY_OTP_310E_LSB;
	}
	else if (reg310f & S5K4E1_SUNNY_MSB)
	{
		return S5K4E1_SUNNY_OTP_310F_MSB;
	}
	else if (reg310f & S5K4E1_SUNNY_LSB)
	{
		return S5K4E1_SUNNY_OTP_310F_LSB;
	}
	else
	{
	...
	return S5K4E1_SUNNY_OTP_MAX;
}



/**************************************************************************
* FunctionName: s5k4e1_sunny_start_otp_read;
* Description : OTP start read process;
***************************************************************************/
static int s5k4e1_sunny_start_otp_read (struct msm_sensor_ctrl_t *s_ctrl)
{
...
/**************************************************************************
* FunctionName: s5k4e1_sunny_update_otp;
* Description : read calculate otp info;
***************************************************************************/
static int s5k4e1_sunny_update_otp (struct msm_sensor_ctrl_t *s_ctrl)
{
	struct s5k4e1_sunny_otp_struct current_otp;
	uint32_t r_gain = 0;
	uint32_t g_gain = 0;
	...
	s5k4e1_sunny_otp_location awb_otp_Location = S5K4E1_SUNNY_OTP_MAX;
	memset(&current_otp, 0, sizeof(struct s5k4e1_sunny_otp_struct));

	/*read*/
	//send read commnad to IC
	s5k4e1_sunny_start_otp_read(s_ctrl);

	//check AWB OTP location
	awb_otp_Location = s5k4e1_sunny_check_awb_otp_location(s_ctrl);
	if (0 != s5k4e1_sunny_read_awb_otp(s_ctrl, awb_otp_Location, &current_otp))
	{
		CMR_LOGE("%s Error!no data in OTP!\n", __func__);
		return -1;
	}
...
/******************************************************************************
Function   :    s5k4e1_sunny_set_otp_info
Description:    check whether we need
******************************************************************************/
int s5k4e1_sunny_p5s07a_otp_func(struct msm_sensor_ctrl_t *s_ctrl, int index)
{
	...
	if (0 == (s5k4e1_sunny_otp_read_flag & S5K4E1_SUNNY_OTP_AWB))
	{
		if(0 != s5k4e1_sunny_update_otp(s_ctrl))
		{
	...
	/*write*/
	s5k4e1_sunny_update_awb_gain(s_ctrl, gr_gain, gg_gain, gb_gain);
