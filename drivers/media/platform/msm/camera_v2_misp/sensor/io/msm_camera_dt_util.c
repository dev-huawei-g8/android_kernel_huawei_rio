	...
	uint16_t gpio_array_size)
{
	...
	if (rc != -EINVAL) {
		...
		rc = 0;
	rc = of_property_read_u32(of_node, "qcom,gpio-misp-vdd", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_err("%s:%dread qcom,gpio-misp-vdd failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-misp-vdd invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_MISP_VDD] =
			gpio_array[val];
		gconf->gpio_num_info->valid[SENSOR_GPIO_MISP_VDD] = 1;
		pr_err("%s qcom,gpio-cam-id %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_MISP_VDD]);
	} else
		rc = 0;
	rc = of_property_read_u32(of_node, "qcom,gpio-af-en", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_err("%s:%dread qcom,gpio-af-en failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-af-en invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_AF_EN] =
			gpio_array[val];
		gconf->gpio_num_info->valid[SENSOR_GPIO_AF_EN] = 1;
		pr_err("%s qcom,gpio-af-en %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_AF_EN]);
	} else
		rc = 0;
	rc = of_property_read_u32(of_node, "qcom,gpio-ois-en", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_err("%s:%dread qcom,gpio-ois-en failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-ois-en invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
		...
		case SENSOR_CLK:
			...
			if (ps)
				...
				pr_err("%s error in power up/down seq data\n",
								__func__);
			if (pd->seq_val == SENSOR_MINIISP_MCLK) {
				misp_set_reset_gpio(0); /*RESET_GPIO need to power down while MCLK power down*/
			}
			break;
		case SENSOR_GPIO:
