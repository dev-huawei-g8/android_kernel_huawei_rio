	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	if(slave_info->sensor_cam_id){
		...
		camera_info->otp_vendor_id = cam_id->vendor_id;
		CDBG("%s expect CAMID %d, vendor id %d\n",slave_info->sensor_name,camera_info->mcam_id,camera_info->otp_vendor_id);
	}else{
		CDBG("%s no need to match CAMID",slave_info->sensor_name);
		camera_info->mcam_id = 2;
		camera_info->otp_vendor_id = 0;
	}
	...
	*  sensor control structure */
	...
	if(otp_index >=0){
		otp_info = kzalloc(sizeof(*otp_info), GFP_KERNEL);
		if(!otp_info){
			pr_err("%s failed: no memory for otp_info\n", __func__);
			rc = -ENOMEM;
			goto free_camid;
		}
		memset(otp_info, 0, sizeof(*otp_info));
		otp_info->otp_size = otp_data_lists[otp_index].otp_info->otp_size;
		otp_info->af_otp.index_start = otp_data_lists[otp_index].otp_info->af_otp.index_start;

		misp_get_otp(otp_info, (uint8_t)slave_info->sensor_init_params.position);
		memcpy(otp_data_lists[otp_index].otp_info, otp_info, sizeof(*otp_info));
	}
	*/
	//set otp info
...
static int32_t msm_sensor_driver_get_dt_data(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (of_property_read_string(of_node, "qcom,misp-firmware-name",
				&s_ctrl->misp_firmware_name) < 0) {
		s_ctrl->misp_firmware_name = NULL;
		pr_err("%s: misp firmware couldn't be found\n",__func__);
	}
	else
	{
		pr_info("%s misp firmware = %s\n", __func__, s_ctrl->misp_firmware_name);
	}
	if (of_property_read_string(of_node, "qcom,misp-boot-firmware-name",
				&s_ctrl->misp_boot_firmware_name) < 0) {
		s_ctrl->misp_boot_firmware_name = NULL;
		pr_err("%s: misp boot-firmware couldn't be found\n",__func__);
	}
	else
	{
		pr_info("%s misp boot-firmware = %s\n", __func__, s_ctrl->misp_boot_firmware_name);
	}
	return rc;
...
int32_t msm_get_probe_sensor_codes(void *setting)
{
	...
	{
		pr_err("%s: fail to get sensor_codes_info\n",__func__);
		return -1;
	}
	...
	memcpy(sensor_codes_info->product_name,s_ctrl->product_name,sizeof(char)*length);
	return 0;
}
