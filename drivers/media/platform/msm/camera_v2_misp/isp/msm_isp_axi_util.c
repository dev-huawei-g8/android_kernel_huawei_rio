	...
	uint8_t input_src)
{
	...
	for (i = 0; i < MAX_NUM_STREAM; i++) {
		...
		if (stream_info->state != ACTIVE)
			continue;
		ISP_DBG("%s: stream type: %d, stream src: %d, frame id: %d, init_frame_drop:%d, runtime_burst_frame_count: %d\n",
			__func__,
...
static void msm_isp_process_done_buf(struct vfe_device *vfe_dev,
	...
	struct msm_isp_timestamp *ts, struct timeval eof_time)
{
	int rc;
	...
	if (buf && ts) {
		...
		if (stream_info->buf_divert && rc == 0 &&
			...
			if (rc == 0) {
				...
				buf_event.mono_timestamp = eof_time;
	...
	struct msm_isp_timestamp *ts)
{
	...
	static struct timeval eof_time;
	if (irq_status0 & (1 << 1))
		eof_time = ts->event_time;
		...
		if (comp_mask & (1 << i)) {
			if (!comp_info->stream_handle) {
				...
				if (done_buf && !rc)
					...
					stream_info, done_buf, ts, eof_time);
			}
		}
