...
Copyright 2014 Analog Devices Inc. <br>
...
*/
#define CHECK_CONV_EN(int_ctrl)		(!((int_ctrl) & 0x100))
/**
...
static int adux1050_debug_mask = 3;
module_param_named(adux1050_debug, adux1050_debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP);
...
#define ADUX1050_ERR(x,arg...) do {\
	if  ((adux1050_debug_mask >= 0)) \
		printk("[CAP_ERROR]"  x"\n",##arg);\
	} while (0)

#define ADUX1050_WARN(x,arg...) do {\
	if ((adux1050_debug_mask >= 1)) \
		printk("[CAP_WARN]"  x"\n",##arg);\
	} while (0)

#define ADUX1050_INFO(x,arg...) do {\
	if ((adux1050_debug_mask >= 2) &&(KERNEL_HWFLOW)) \
		printk("[CAP_INFO]"  x"\n",##arg);\
	} while (0)

#define ADUX1050_DEBUG(x,arg...) do {\
	if ((adux1050_debug_mask >= 3) && (KERNEL_HWFLOW)) \
		printk("[CAP_DEBUG]"  x"\n",##arg);\
	} while (0)
